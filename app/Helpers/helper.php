    ...
    function sessionCartToDatabase():void {
        if (Cart::content()->count() > 0 && auth()->check()) {
            ...
            foreach ($carts as $item) {
                ...
                if($course && !isOwnCourse($user, $course) && !hasCourseInCartOrPurchased($user, $course)){
...
if (!function_exists('hasCourseInCartOrPurchased')) {
    function hasCourseInCartOrPurchased($user, $course) {
        return $user->enrollments()->where('course_id', $course->id)->exists() ||
        $user->carts()->where('course_id', $course->id)->exists();
    }
}