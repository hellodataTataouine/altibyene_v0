...
class CoursController extends Controller
{
    function index() : View {
        ...
        return view('frontend.pages.cours', compact('categories', 'languages', 'levels'));
    }
    ...
    function show(string $slug) {
        $cours = Cours::active()->with(['chapters' => function($query) {
            $query->orderBy('order', 'asc')->with(['chapterItems', 'chapterItems.lesson', 'chapterItems.quiz']);
        }])
        ->withCount(['reviews' => function($query) {
            $query->where('status', 1)->whereHas('cours')->whereHas('user');
        }])
        ->where('slug', $slug)->firstOrFail();
        $courseLessonCount = CourseChapterLesson::where('cours_id', $cours->id)->count();
        $courseQuizCount = Quiz::where('cours_id', $cours->id)->count();
        $reviews = CourseReview::where('cours_id', $cours->id)->where('status', 1)->whereHas('cours')->whereHas('user')->orderBy('created_at', 'desc')->paginate(20);
        return view('frontend.pages.course-details', compact('cours', 'courseLessonCount', 'courseQuizCount', 'reviews'));
    }
}
